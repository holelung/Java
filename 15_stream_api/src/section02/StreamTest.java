...
public class StreamTest {
	public static void main(String[] args) {
		...
		// 참조하는 객체가 null 이 됐을 때 사용하지 않도록 함		
		...
		System.out.println();
		// numbers 에서 짝수 합 구하기
		int evenSum = numbers.stream().filter(num -> num%2==0).reduce(0, Integer::sum);
		System.out.println("짝수 합 : " + evenSum);
		
		
		/* map() : 
		 * - stream의 모든 요소를 새로운 형태로 변환 후
		 *   새 스트림으로 반환
		 ...
		 * */ 		
		List<Integer> list = Arrays.asList(12, 44, 3, 55, 13, 73, 39);
		System.out.println("\n정렬");
		list.stream().sorted().forEach(num ->  System.out.print(num + " ")); // 정렬
