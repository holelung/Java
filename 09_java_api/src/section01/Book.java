...
public class Book {
	...
	/* equals 오버라이딩 
	 * -> 비교하려는 두 객체의 필드 값이 같은지 비교(동등비교)
	 * */
	@Override
	public boolean equals(Object obj) {
		
		// 같은 객체를 참조하고 있는 경우(주소가 같은경우)
		// -> 같은 객체 비교하는데 시간낭비 하지말자
		...
		// 현재 객체와 전달 받은 객체의 bookNumber 다르면
		if(this.bookNumber != other.bookNumber) {
			return false;
		}
		
		// 현재 객체와 전달받은 객체의 title가 다를 경우
		if(!this.title.equals(other.title)){
			return false;
		}
		
		// 현재 객체와 전달받은 객체의 equals가 다를 경우
...
//		int result = 1;
//		final int PRIME = 31; // 컴퓨터에서 연산속도가 제일 빠른 소수
//		
