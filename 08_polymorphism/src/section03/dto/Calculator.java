 ...
 * 별도의 필드/메서드를 작성하지 않는다(약속, 규약)
 *  -> 언제든지 동알한 코드로 실행될 수 있게 하려고
 *  
 * - 인터페이스 작성법( []는 생략가능)
 * 1) 상수형 필드만 작성 가능
 ...
 * 		-> [public abstract] 반환명 메서드명();
 * 
 * 3) default 메서드 작성가능(오버라이딩 가능)
 * 		-> default void 메서드명(){}
 * 
 * - 인터페이스 사용법
 ...
 * -> 사용 방법은 추상 클래스와 동일
 * 	1. 인터페이스는 객체 생성 불가
 *  2. 부모 타입 참조 변수로는 사용가능
 */

/** Calculator 인터페이스
 * -> 객체 생성 용도 X, 클래스/객체간의 접점 용도O
 */
public interface Calculator {
	/* field는 public static final 만 작성가능 */
	...
	/**
	 * <pre>
	 * <strong>전달 받은 num이 최대/최소를 초과하는지 확인</strong>
	 * - 최대값 초과 시 MAX_NUM 반환
	 * - 최소값 미만 시 MIN_NUM 반환
	 * - 범위 내 숫자는 그냥 반환
	 * </pre>
