...
public class AbstractionRun {
	public static void main(String[] args) {
		...
		// 다형성 - 업 캐스팅 적용
		Animal a2 = new Cat("장모");

		// Animal의 eat() 메서드는 추상메서드로
		// 원래는 호출이 불가능해야 하지만
		// 참조하는 객체가 eat()을 구현한 자식 타입으로
		// 컴파일 시(정적 바인딩) 오류가 발생하지 않는다

		// Cat의 eat()메서드가 실행됨(동적 바인딩)
		System.out.println("-------------------------");
		...
		/* 확인 4. 추상 클래스 활용법 */
		printAnimal(new Cat("단모"));
		printAnimal(new Snake(true));
	...
	public static void printAnimal(Animal animal) {
	 /* 매개 변수 Animal(추상 클래스)
		...
		*   
		* - Animal 이 물려준 추상 메서드를 호출(동적 바인딩)
		*/ 
		System.out.println("-------------------------------");
